...
class AvionEnv2D:
	...
	def _get_state(self):
		...
		dx_norm = np.clip(dx / 5000.0, -1.0, 1.0)
		dy_norm = np.clip(dy / 3000.0, -1.0, 1.0)
		err_angle_norm = erreur_angle / math.pi
		vx_norm = self.vx / 100.0
		vy_norm = self.vy / 50.0
		return np.array([dx_norm, dy_norm, err_angle_norm, vx_norm, vy_norm], dtype=np.float32)
	...
	def step(self, action):
		...
		recompense = 5.0 - (5.0 * (erreur_angle / math.pi)) - (dist_actuelle / 1500.0)

		if dy < -200.0 and self.vy > 5.0:
			recompense -= 15.0
		if dx < -200.0 and self.vx < -5.0:
			recompense += 10.0

		termine = self.y <= 0 or self.y > 6000.0 or self.x > 12000.0 or self.x < -500.0

		if dist_actuelle < 300.0:
			recompense += 1500.0
...
for ep in range(700):
	...
	if (ep + 1) % 50 == 0:
		print(f"Episode {ep+1:03d}/700 | Score: {total_reward:.1f} | Epsilon: {agent.epsilon:.3f} | WP Reach: {env.current_wp_idx}/{len(env.waypoints)} | Final Pos: X={env.x:.0f}m, Y={env.y:.0f}m")
